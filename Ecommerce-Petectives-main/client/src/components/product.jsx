...
const Product = ({ product, addItem, cartItems /* , reviews, getReviewsId  */ }) => {
	...
	return (
		<div className="card">
			...
			{console.log("ip", ip)}
			{product.stock > 1 ? 
				<button id= 'addToCartButton' type='button' onClick={() => addItem({ product: product, ip: ip })} >
				Add To Cart
				</button>
				: 
				<button id= 'disabledButton' type='button' onClick={() => {console.log('no hace nada')}} disabled={false}>
				Sold Out
				</button>
			}
		</div>
